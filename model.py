...
class faster_rcnn(object):
	...
	def train_model(self,MAX_STEP,img_path_list,gt_boxes_list,pre_train=None,save_step=10000,log_inter=10,base_eta=1e-3,decay_steps=50000):
		...
		model_save_path = 'model-step{}-decay{}-eta{}'.format(MAX_STEP,decay_steps,base_eta)
		...
		for img_batch,im_info_batch,gt_boxes_batch in get_batch():
			...
			if step % save_step == 0:
				self._save_para(sess,'{}/faster_rcnn-{}.npz'.format(model_save_path,step))
				...
				plt.savefig('{}/FasterRcnn_Loss.eps'.format(model_save_path),format='eps')
			if step >= MAX_STEP:
				self._save_para(sess,'{}/faster_rcnn-{}.npz'.format(model_save_path,step))
				print('\nFinished')
				break
	...
	def test_model(self,img_path_list,model_path,thresh=0.05,nms_th=0.3,nms_mode='Union'):
		...
		for index,img_path in enumerate(img_path_list):
			...
			img = cv2.resize(img,None,None,fx=1/ts,fy=1/ts,interpolation=cv2.INTER_LINEAR)
			img = [img]
	...
	def network(self,data,im_info,gt_boxes=None,net_type='TRAIN'):
		...
		#========== RCNN ==========
		# output
		pool5 = self.roi_pool(feature_map,rois,'pool5')
		net = tf.reshape(tf.transpose(pool5,[0,3,1,2]),[-1,7*7*512])
		net = self.fc_layer(net,'fc6',7*7*512,4096)
		if net_type == 'TRAIN':
	...
	def roi_pool(self,feature_map,rois,name):
		return roi_pool_op.roi_pool(feature_map,rois,7,7,1./16,name)[0]
